...
class Graphe_couleur : public Graphe{
	...
	void backtracking(){
		...
		// sommet = ...
		// couleur = ...
		bool boo=recursive(0);
		std::cout << "l'algo renvoie" << boo << std::endl;
		;}
		
	bool recursive(int cont){ // Attention choisir avec arguments ou sans arguments (essayer sans)
	cont+=1;
...
std::cout << "              " << std::endl;
		...
		for(int i=0;i<n;i++){
			if(couleurs[i]==-1){
			    v[i]=nb_color_disp(i);
			std::cout << nb_color_disp(i) << std::endl;
		...
		std::cout << "sommets par nb coul disp ? numero:" << std::distance(v.begin(),it) << std::endl;
		
		std::cout << "BOUCLE cas egalite" << std::endl;
		
		while(it!=v.begin()+n-1){
		it=min_element(it+1,v.end());
		myints.push_back(std::distance(v.begin(),it));
		std::cout << "sommets par nb coul disp ? numero:" << std::distance(v.begin(),it) << std::endl;
		}
		
				std::cout << "BOUCLE cas egalite finie" << std::endl;
		...
		if(myints.size() > 1) {
			...
			for(int i=0;i<myints.size();i++){
				...
				std::cout << "sommets mm nb couleurs, nb voisins ? numero:" << myints[i] << "voisins " << tab1[myints[i]] << std::endl;}
		...
		std::cout<< "cas egalite fini" << std::endl;
		
		std::cout<< "sommet apres cas egalite" << sommet_courant << std::endl;
		...
		std::cout<< "COULEURS DES DEUX " << couleurs[0] << couleurs[1] << couleurs[2] << couleurs[3] << std::endl;




		for(int i=1;i<nb_color_disp(sommet_courant);i++){
		bool b=recursive(cont);
		if(b){return true;}
